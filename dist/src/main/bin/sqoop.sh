...
case $COMMAND in
  ...
  client)
    ...
    ${EXEC_JAVA} ${CUSTOM_AUTH_OPTS} -Djava.security.auth.login.config=${MapRHomeDir}/conf/mapr.login.conf -classpath ${CLASSPATH} org.apache.sqoop.shell.SqoopShell $SCRIPT_NAME
    ;;
